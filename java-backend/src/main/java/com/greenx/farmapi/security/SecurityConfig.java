    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        // Public endpoints
                        .requestMatchers("/api/auth/**", "/api/health", "/api/land-registration/submit").permitAll()
                        .requestMatchers("/api/ai/status").permitAll()
                        .requestMatchers("/api/ws/**").permitAll()
                        .requestMatchers(HttpMethod.OPTIONS, "/**").permitAll()
                        // AI endpoints — require authentication (except /status above)
                        .requestMatchers("/api/ai/**").authenticated()
                        // Role-specific path rules
                        .requestMatchers("/api/admin/**").hasAnyRole("ADMIN", "CLUSTER_ADMIN")
                        .requestMatchers("/api/expert/**").hasRole("EXPERT")
                        .requestMatchers("/api/field/**").hasRole("FIELD_MANAGER")
                        .requestMatchers("/api/fieldmanager/**").hasRole("FIELD_MANAGER")
                        .requestMatchers("/api/worker/**").hasRole("WORKER")
                        .requestMatchers("/api/land/**").hasAnyRole("LANDOWNER", "LAND_OWNER")
                        .requestMatchers("/api/landowner/**").hasAnyRole("LANDOWNER", "LAND_OWNER")
