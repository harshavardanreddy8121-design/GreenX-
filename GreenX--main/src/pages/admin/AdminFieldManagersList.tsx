...
export default function AdminFieldManagersList() {
  ...
  const { data: managers = [], isLoading, isError, error, refetch } = useQuery({
    queryKey: ['admin-field-managers-list'],
    ...
    <>
      <div className="gx-page-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            <div className="gx-page-title"><Tractor className="inline-block w-4 h-4 mr-1 align-middle" /> Field Managers</div>
            <div className="gx-page-sub">{managers.length} total field managers</div>
          </div>
        </div>
      </div>
      ...
      {isError && !isLoading && (
        <div className="gx-alert-box gx-alert-red" style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <strong>Failed to load field managers:</strong> {(error as Error)?.message || 'Unknown error'}
          </div>
      ...
      {!isLoading && !isError && (
        <div className="gx-card">
          <div className="gx-card-header">
            <div className="gx-card-title"><Tractor className="inline-block w-4 h-4 mr-1 align-middle" /> All Field Managers</div>
          ...
          <div className="gx-card-body">
            {paginated.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '40px 0', opacity: 0.5 }}>
                ...
                <div>{search || statusFilter !== 'ALL' ? 'No field managers match your search.' : 'No field managers found.'}</div>
              </div>
            ) : (
                ...
                <tbody>
                  {paginated.map((m: any, i: number) => (
                    ...
                    >
                      ...
                      <td><code style={{ fontSize: 11 }}>{m.uid || '—'}</code></td>
                      <td>{m.email || '—'}</td>
                      <td>{m.phone || '—'}</td>
                      <td>
                        <span className={m.isActive === false ? 'gx-status gx-s-alert' : 'gx-status gx-s-done'}>
