...
export default function AdminExpertsList() {
  ...
  return (
    <AdminListPage
      title="Experts"
      subtitle={`${data.length} soil & crop experts`}
      ...
      onRowClick={row => navigate(`/admin/experts/${(row as any).id}`)}
      emptyMessage="No experts found. Try adjusting your search or filters."
    />
  );
}
