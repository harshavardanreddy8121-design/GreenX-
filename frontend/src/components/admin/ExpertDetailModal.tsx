...
import { X, Microscope, MapPin, FileText, TestTubes, User, Phone, Mail, Hash } from 'lucide-react';
...
export default function ExpertDetailModal({ expertId, onClose }: Props) {
  const { data, isLoading, error } = useQuery({
    queryKey: ['admin-expert-detail', expertId],
    queryFn: () => admin.getExpertDetail(expertId),
  });

  const user = data?.user as any;
  const farms = (data?.assignedFarms as any[]) ?? [];
  const reports = (data?.soilReports as any[]) ?? [];
  const pendingSamples = (data?.pendingSamples as any[]) ?? [];
      ...
      <div style={modalStyle} onClick={e => e.stopPropagation()}>
        ...
        <div style={headerStyle}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <Microscope style={{ width: 20, height: 20, color: 'var(--gx-blue)' }} />
            <span style={{ fontWeight: 700, fontSize: 16 }}>Expert Detail</span>
          </div>
          <button onClick={onClose} style={closeBtnStyle}><X style={{ width: 16, height: 16 }} /></button>
        </div>

        <div style={bodyStyle}>
          {isLoading && <div style={centerStyle}>Loading expert details…</div>}
          ...
          {data && (
            <>
              ...
              <div style={statsRowStyle}>
                <StatBox label="Farms" value={stats.totalFarms ?? 0} color="var(--gx-blue)" />
                <StatBox label="Reports" value={stats.totalReports ?? 0} color="var(--gx-green)" />
                <StatBox label="Total Samples" value={stats.totalSamples ?? 0} color="var(--gx-gold)" />
                <StatBox label="Pending" value={stats.pendingSamples ?? 0} color="var(--gx-orange)" />
              </div>
              ...
              <div style={sectionStyle}>
                ...
                {reports.length === 0 ? <EmptyMsg text="No reports created" /> : (
                  <table style={tableStyle}>
              ...
              {/* Pending Samples */}
              <div style={sectionStyle}>
                <div style={sectionTitleStyle}><TestTubes style={iconSm} /> Pending Samples ({pendingSamples.length})</div>
                {pendingSamples.length === 0 ? <EmptyMsg text="No pending samples" /> : (
                  <table style={tableStyle}>
                    <thead><tr style={thRowStyle}><th style={thStyle}>Code</th><th style={thStyle}>Farm</th><th style={thStyle}>Priority</th><th style={thStyle}>Status</th></tr></thead>
                    <tbody>
                      {pendingSamples.slice(0, 8).map((s: any) => (
                        <tr key={s.id} style={trStyle}>
                          <td style={tdStyle}>{s.sampleCode || s.id}</td>
                          <td style={tdStyle}>{s.farmId}</td>
                          <td style={tdStyle}><StatusBadge status={s.priority} /></td>
...
const infoGridStyle: React.CSSProperties = { display: 'flex', flexDirection: 'column', gap: 2 };
